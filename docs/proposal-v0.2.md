...
**实例身份策略三设置**(管理员可配):①`allow_root` 是否作为根节点(否则拒绝本地注册,仅承载宾客身份与内容);②根节点注册门槛 ⊆ {邮箱, 手机(保留位,未接 SMS 通道时明确报错), 注册码};③`trusted_identity_servers` 承认哪些服务器的身份(`*` = 全部),联邦会话在验签前按此准入且不对名单外域发起密钥拉取。
