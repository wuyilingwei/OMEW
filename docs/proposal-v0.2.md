...
**服务器级角色层**:server_owner(首用户,唯一)> 据点 owner ≈ server_admin > 用户;服务器 owner/admin 默认持有全部据点的管理身份(视同据点 owner,唯所有权转让除外);实例本地,不联邦。
**实例策略**(全部为部署 env 开关,运营者在 wrangler 配置层设定,无运行时改写端点;admin UI 只读展示):①`allow_root` 是否作为根节点(否则拒绝本地注册,仅承载宾客身份与内容);②根节点注册门槛 ⊆ {邮箱, 手机(保留位,未接 SMS 通道时明确报错), 注册码};③`trusted_identity_servers` 承认哪些服务器的身份(`*` = 全部),联邦会话在验签前按此准入且不对名单外域发起密钥拉取;④`federation_peers` 要加入的已知服务器(内容对等出站白名单,与③的身份准入相互独立);⑤`stronghold_creation` 建据点策略 = open 公开 / restricted 特定人名单 / application 需申请审批。
...
**授权现状**(2026-08-19 运营者一手说明，2026-08-22 进一步确认商业使用边界):Mew **Logo/字标为受保护资产**,不使用;**Mew 自有表情包与官方插画**仅获非商业使用许可，可在符合根目录 `ASSET-LICENSE.md` 的非商业实例中使用(官方吉祥物插画 76+116 文件已确认本地可恢复)。商业部署可继续使用 AGPL 代码，但必须移除或替换 OMEW Logo 与 Mew 官方美术。第三方 IP 包不属该许可范围:aru(阿鲁)署名画师微博 @\_SiC\_、2233 娘系 bilibili 官方 IP、萌百娘系萌娘百科——均不使用。
