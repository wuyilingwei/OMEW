...
- **授权边界(运营者一手说明,2026-08-19;2026-08-22 进一步确认商业使用边界)**:Mew Logo/字标为受保护资产,不使用不分发;Mew 自有表情包与吉祥物插画等仅获非商业使用许可，具体定义、纯捐助例外与商业部署移除义务以根目录 `ASSET-LICENSE.md` 为准;
- 第三方 IP 表情包(2233 娘 = bilibili、阿鲁 = 画师 @_SiC_、萌百娘 = 萌娘百科)不属 Mew 运营方可授权范围,维持不使用;
- 想法配图为其他用户 UGC、Tomon 贴纸与 Mew 无关,均不作设计参照;
- 自有插画 109 张已按权利人指示入仓(assets/mew/ 附 NOTICE);Logo、第三方 IP、UGC、截图不入仓,提取件存本地审计目录。
