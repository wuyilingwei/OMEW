...
<template>
  <section class="direct-message-panel" :aria-label="`与 ${targetName} 私聊`">
    ...
    <p v-else-if="error" class="direct-message-panel__error" role="alert">{{ error }}</p>
    <p v-else-if="messages.length === 0" class="direct-message-panel__note">还没有消息，发送第一句问候吧。</p>
    <ol v-else class="direct-message-panel__messages">
      <li v-for="message in messages" :key="message.id" class="direct-message-panel__message">
    ...
    <form class="direct-message-panel__compose" @submit.prevent="send">
      ...
      <WinButton type="submit" Style="AccentButtonStyle" :disabled="!draft.trim() || sending">{{ sending ? '发送中…' : '发送' }}</WinButton>
    </form>
  </section>
</template>
...
.direct-message-panel__compose textarea { min-width: 0; flex: 1; resize: vertical; padding: .4rem .5rem; border: 1px solid var(--control-stroke); border-radius: var(--radius-sm); background: var(--control-fill); color: var(--text-primary); font: inherit; font-size: .74rem; }
</style>
