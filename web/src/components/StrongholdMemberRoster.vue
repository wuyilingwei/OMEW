...
const { isReadOnly, selectedNodeId } = useStronghold()

const roleLabels = { owner: '据点所有者', mod: '管理员', member: '成员' } as const
const searchQuery = ref('')
const expandedActor = ref<string | null>(null)
const visibleMembers = computed(() => {
  const query = searchQuery.value.trim().toLowerCase()
  if (!query) return members.value
  return members.value.filter((member) => [member.display_name, member.username, member.actor]
...
function toggleMember(actor: string) {
  expandedActor.value = expandedActor.value === actor ? null : actor
}
</script>
...
<template>
  <section v-if="!isReadOnly" class="member-roster" aria-labelledby="member-roster-title">
    ...
    <label class="member-roster__search">
      <span class="sr-only">搜索成员</span>
      <input v-model="searchQuery" type="search" placeholder="搜索成员" autocomplete="off" />
    </label>
    ...
    <p v-else-if="visibleMembers.length === 0" class="member-roster__empty">未找到匹配成员</p>
    <ul v-else class="member-roster__list">
      <li v-for="member in visibleMembers" :key="member.actor" class="member-roster__item">
        <button class="member-roster__trigger" type="button" :aria-expanded="expandedActor === member.actor" @click="toggleMember(member.actor)">
        ...
        <MemberInfoCard v-if="expandedActor === member.actor && selectedNodeId" inline :node-id="selectedNodeId" :member="member" @close="expandedActor = null" />
      </li>
    </ul>
  </section>
</template>
...
.member-roster__empty { margin: 0; flex: 0 0 auto; color: var(--text-secondary); font-size: .72rem; }
.member-roster__empty--error { color: var(--critical-text); }
.member-roster__search { display: block; flex: 0 0 auto; }
.member-roster__search input { width: 100%; box-sizing: border-box; padding: .42rem .55rem; border: 1px solid var(--control-stroke); border-radius: var(--radius-sm); background: var(--control-fill); color: var(--text-primary); font: inherit; font-size: .74rem; }
.member-roster__list { min-height: 0; flex: 1 1 auto; margin: 0; padding: 0 .15rem 0 0; overflow-y: auto; list-style: none; }
.member-roster__item { padding: 0; border-bottom: 1px solid var(--stroke-divider); }
.member-roster__trigger { width: 100%; display: grid; grid-template-columns: auto minmax(0, 1fr) auto; align-items: center; gap: .55rem; padding: .48rem 0; border: 0; background: transparent; color: inherit; text-align: left; cursor: pointer; }
.member-roster__trigger:hover { background: var(--subtle-fill-secondary); }
.member-roster__trigger:focus-visible { outline: 2px solid var(--accent-fill); outline-offset: -2px; }
