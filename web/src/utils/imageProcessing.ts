...
export function createCropPreset(aspectRatio: number): CropRect {
  if (!Number.isFinite(aspectRatio) || aspectRatio <= 0) return createFreeCropRect()
  let width = 1; let height = 1 / aspectRatio
  if (height > 1) { height = 1; width = aspectRatio }
  return { x: (1 - width) / 2, y: (1 - height) / 2, width, height }
}

/** Clamps a rectangle and, when supplied, keeps its width/height ratio. */
export function constrainCropRect(rect: CropRect, aspectRatio?: number): CropRect {
  let normalized = normalizeCropRect(rect)
  if (!Number.isFinite(aspectRatio) || !aspectRatio || aspectRatio <= 0) return normalized
...
export function applyMosaic(canvas: HTMLCanvasElement, strokes: MosaicStroke[] = []): void {
  ...
  for (const stroke of strokes) {
    const radius = stroke.radius <= 1 ? Math.max(4, Math.round(stroke.radius * Math.min(canvas.width, canvas.height))) : Math.max(4, Math.round(stroke.radius))
    const x = Math.round(Math.max(0, Math.min(1, stroke.x)) * canvas.width); const y = Math.round(Math.max(0, Math.min(1, stroke.y)) * canvas.height)
    const left = Math.max(0, x - radius); const top = Math.max(0, y - radius)
    const regionWidth = Math.min(canvas.width - left, radius * 2); const regionHeight = Math.min(canvas.height - top, radius * 2)
    if (regionWidth <= 0 || regionHeight <= 0) continue
    const sample = document.createElement('canvas'); sample.width = Math.max(1, Math.ceil(regionWidth / 12)); sample.height = Math.max(1, Math.ceil(regionHeight / 12))
    const sampleCtx = sample.getContext('2d'); if (!sampleCtx) continue
    sampleCtx.drawImage(canvas, left, top, regionWidth, regionHeight, 0, 0, sample.width, sample.height)
    ctx.imageSmoothingEnabled = false; ctx.drawImage(sample, 0, 0, sample.width, sample.height, left, top, regionWidth, regionHeight); ctx.imageSmoothingEnabled = true
  }
}
...
export async function processImage(file: File, options: ImageProcessOptions): Promise<ProcessedImage> {
  ...
  const edited = Boolean(options.crop || options.mosaic?.length)
  ...
  const image = await loadImage(file); const maxEdge = options.outputSize ?? MAX_STATIC_EDGE
  const source = drawContain(image, scaledDimensions(image, Math.min(MAX_STATIC_EDGE, maxEdge))); applyMosaic(source, options.mosaic)
  const cropPixels = { width: source.width * crop.width, height: source.height * crop.height }; const scale = Math.min(1, maxEdge / Math.max(cropPixels.width, cropPixels.height))
  const canvas = drawCrop(source, cropPixels.width * scale, cropPixels.height * scale, crop)
  const requestedMime = outputMime(options.mode, file.type); const encoded = await canvasToBlob(canvas, requestedMime)
...
export async function previewImage(file: File, options: Omit<ImageProcessOptions, 'mode'>): Promise<HTMLCanvasElement | null> {
  ...
  const image = await loadImage(file); const crop = options.crop ? normalizeCropRect(options.crop) : createFreeCropRect(); const maxEdge = options.outputSize ?? MAX_STATIC_EDGE
  const source = drawContain(image, scaledDimensions(image, Math.min(MAX_STATIC_EDGE, maxEdge))); applyMosaic(source, options.mosaic)
  const cropPixels = { width: source.width * crop.width, height: source.height * crop.height }; const scale = Math.min(1, maxEdge / Math.max(cropPixels.width, cropPixels.height))
  return drawCrop(source, cropPixels.width * scale, cropPixels.height * scale, crop)
}

function outputMime(mode: ImageOutputMode, sourceMime: string): string { return mode === 'smart' ? automaticOutputMime(sourceMime, supportsWebpEncoding()) : sourceEncodingMime(sourceMime) }
function supportsWebpEncoding(): boolean { try { return document.createElement('canvas').toDataURL('image/webp').startsWith('data:image/webp') } catch { return false } }
function scaledDimensions(image: HTMLImageElement, maxEdge: number): { width: number; height: number } { const scale = Math.min(1, maxEdge / Math.max(image.naturalWidth, image.naturalHeight)); return { width: Math.max(1, Math.round(image.naturalWidth * scale)), height: Math.max(1, Math.round(image.naturalHeight * scale)) } }
function drawContain(image: HTMLImageElement, dimensions: { width: number; height: number }): HTMLCanvasElement { const canvas = document.createElement('canvas'); canvas.width = dimensions.width; canvas.height = dimensions.height; const ctx = canvas.getContext('2d'); if (!ctx) throw new Error('CANVAS_UNAVAILABLE'); ctx.imageSmoothingEnabled = true; ctx.imageSmoothingQuality = 'high'; ctx.drawImage(image, 0, 0, dimensions.width, dimensions.height); return canvas }
function sourceEncodingMime(mime: string): string { return ENCODABLE_SOURCE_MIMES.has(mime) ? mime : 'image/png' }
function canvasToBlob(canvas: HTMLCanvasElement, mime: string): Promise<Blob> { return new Promise((resolve, reject) => canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('IMAGE_ENCODE_FAILED')), mime, 0.9)) }
function loadImage(file: Blob): Promise<HTMLImageElement> { return new Promise((resolve, reject) => { const url = URL.createObjectURL(file); const image = new Image(); image.onload = () => { URL.revokeObjectURL(url); resolve(image) }; image.onerror = () => { URL.revokeObjectURL(url); reject(new Error('IMAGE_DECODE_FAILED')) }; image.src = url }) }
